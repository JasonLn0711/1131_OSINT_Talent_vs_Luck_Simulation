...
本專案是以 Python 實現的模擬程式，作為國立陽明交通大學學生的課程作業，基於 A. Pluchino、A. E. Biondo 和 A. Rapisarda 的論文 [*Talent vs Luck: The Role of Randomness in Success and Failure*](https://arxiv.org/abs/1802.07068) 所提出的概念。專案旨在呈現成功受天賦與運氣共同影響的情況，並說明為什麼人們常常會被投資詐騙所欺騙。
...
- `talent_vs_luck_simulation.py`：主 Python 腳本，運行模擬並呈現天賦與運氣如何互動促成成功。
- `requirements.txt`：列出運行 Python 程式所需的依賴包。
- `README.md`：本文件提供專案概述。
- `experiement_talent_vs_luck__v2_1`：Jupyter Notebook，包含完整的模擬代碼、步驟解說和顯示運氣對成功影響的視覺化圖表。
- **視覺化呈現**：腳本會生成多張圖表，呈現天賦分佈、財富累積以及運氣對成功的影響。
...
3. **運氣對財務成功的影響**：透過模擬運氣對成功的影響，本專案呈現了為何追隨看似成功的投資者可能具有誤導性。這樣的成功或許更與身處對的時間與地點有關，而非真正的技能。
...
此模擬強而有力地呈現了人們可能將隨機的成功誤認為是技能的情形。以下是為何這在投資詐騙背景下至關重要：
...
本專案為開源專案，並以 NYCU License 授權。您可以在對原始論文和本專案的合理引用的前提下，自由使用、修改和分發此代碼。
