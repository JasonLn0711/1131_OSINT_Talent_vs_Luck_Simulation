...
本專案是以 Python 實現的模擬程式，作為國立陽明交通大學學生的課程作業，基於 A. Pluchino、A. E. Biondo 和 A. Rapisarda 的論文 [*Talent vs Luck: The Role of Randomness in Success and Failure*](https://arxiv.org/abs/1802.07068) 所提出的概念。專案旨在展示成功受天賦與運氣共同影響的情況，並說明為什麼人們常常會被投資詐騙所欺騙。
...
3. 可視化展示說明，揭示了為何表面上的成功往往與運氣相關，而非純粹技能，並解析詐騙者如何利用這些錯覺。

## 專案內容

- `talent_vs_luck_simulation.py`：主 Python 腳本，運行模擬並展示天賦與運氣如何互動促成成功。
- `requirements.txt`：列出運行 Python 程式所需的依賴包。
- `README.md`：本文件提供專案概述。
- `experiement_talent_vs_luck__v2_1`：Jupyter Notebook，包含完整的模擬代碼、步驟解說和顯示運氣對成功影響的可視化圖表。
- **可視化展示**：腳本會生成多張圖表，展示天賦分佈、財富累積以及運氣對成功的影響。

## 專案如何與投資詐騙相關

### 功績主義的幻覺

論文指出，成功常常被單純歸因於個人的天賦和努力，這種觀點被稱為「天真的功績主義」。事實上，運氣起著重要作用。詐騙者利用這一錯誤認知，假裝他們的成功來自於技能，而隱藏運氣或甚至欺騙在其中扮演的角色。

### 為何人們會被詐騙欺騙

1. **錯誤歸因成功**：人們常常將運氣誤認為技能。在投資詐騙中，騙徒透過塑造保證成功的故事來利用這一點，讓人們誤以為他們的方法是萬無一失的。
2. **敘事謬誤**：人們往往會圍繞成功創造出令人信服的故事，忽略了運氣的角色。這使得詐騙故事（通常承諾穩定且高回報）更容易被相信。
3. **運氣對財務成功的影響**：透過模擬運氣對成功的影響，本專案展示了為何追隨看似成功的投資者可能具有誤導性。這樣的成功或許更與身處對的時間與地點有關，而非真正的技能。

## 運行程式

### 先決條件

請確保您已安裝 Python 3.x，並且安裝 `requirements.txt` 中列出的依賴包：
...
1. 將此資料庫克隆到您的本地端：
   ...
   git clone https://github.com/JasonLn0711/talent-vs-luck-simulation.git
   ```
2. 打開 Jupyter Notebook 文件 `experiement_talent_vs_luck_2.ipynb`：
   ```bash
   jupyter notebook experiement_talent_vs_luck_2.ipynb
   ```
3. 按照 notebook 中的指示逐步運行每個單元格，這將引導您完成模擬過程並生成結果的可視化。
...
此模擬強而有力地展示了人們可能將隨機的成功誤認為是技能的情形。以下是為何這在投資詐騙背景下至關重要：
...
- **實用建議**：透過理解此模擬的結果，您將更有能力識別假投資方案的警示信號。
...
如果您想為此專案做出貢獻，請分支此資料庫並提交 pull request。歡迎所有貢獻！

## 授權

本專案為開源專案，並以 NYCU License 授權。您可以自由使用、修改和分發此代碼，前提是包含對原始論文和本專案的歸屬。
